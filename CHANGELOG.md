...
- **`sniff_discover` MCP response enriched** with `detectedFrom: { method, detail }` and `reports: { html?, json?, junit? }` so callers no longer need to grep `savedPaths` by extension or shell out for `sniff-reports/discovery/*`. `savedPaths` kept for back-compat. `detectedFrom.method` is one of `flag | config | env | script | framework | probe | none` — explains how sniff chose the baseUrl.
